...
import { publicApi, publicAssetUrl } from "../lib/api";
import { calculatePromotions, getProductPromotionPreview } from "../lib/promotions";
import type { Branch, Category, ProductServiceChannel, Promotion } from "../types";
...
type PublicMenu = { business: { id: number; slug: string; name: string; logo_url?: string | null }; branch: Branch; categories: Category[]; products: PublicProduct[]; promotions: Promotion[] };
...
export function PublicStorePage() {
  ...
  const promotionChannel: ProductServiceChannel = fulfillment === "delivery" ? "digital_delivery" : "digital_takeaway";
  const promotionCalculation = calculatePromotions(
    cart.map((item) => ({
      productId: item.id,
  ...
  const subtotal = promotionCalculation.subtotal;
  const productTotal = promotionCalculation.total;
  const total = productTotal + (fulfillment === "delivery" ? menu.branch.delivery_fee : 0);
  ...
  return <main className="public-store">
    ...
    <section className="store-content"><div className="store-categories"><button className={category === "all" ? "active" : ""} onClick={() => setCategory("all")}>Todo</button>{menu.categories.map((item) => <button key={item.id} className={category === item.id ? "active" : ""} onClick={() => setCategory(item.id)}>{item.name}</button>)}</div><div className="store-product-grid">{products.map((product) => {
      const inCart = cart.find((item) => item.id === product.id);
      const promotion = getProductPromotionPreview({ productId: product.id, categoryId: product.category_id, unitPrice: product.price }, menu.promotions ?? [], promotionChannel);
      return <article key={product.id}><div className="store-product-image" style={product.image_url ? { backgroundImage: `url(${publicAssetUrl(product.image_url)})` } : undefined}>{!product.image_url && <ChefHat />}{promotion.label && <span className="store-promotion-badge">{promotion.label}</span>}</div><div><span className="eyebrow">Hecho en casa</span><h2>{product.name}</h2><p>{product.description || "Preparado con ingredientes frescos."}</p><footer><strong className={promotion.promotionalPrice !== null ? "store-promotional-price" : ""}>{product.variants.length > 1 && <small>Desde </small>}{promotion.promotionalPrice !== null && <del><Money value={product.price} /></del>}<Money value={promotion.promotionalPrice ?? product.price} /></strong>{inCart ? <div className="public-qty"><button onClick={() => change(product, -1)}><Minus /></button><span>{inCart.quantity}</span><button onClick={() => change(product, 1)}><Plus /></button></div> : <button className="public-add" onClick={() => change(product, 1)}><Plus /> Agregar</button>}</footer></div></article>;
    })}</div></section>
    {count > 0 && <button className="floating-cart" onClick={() => setCartOpen(true)}><span><ShoppingBag /> {count} producto(s)</span><strong>Ver pedido · <Money value={productTotal} /></strong></button>}
    {cartOpen && <div className="public-drawer-backdrop" onClick={(event) => event.target === event.currentTarget && setCartOpen(false)}><aside className="public-cart"><header><div><span className="eyebrow">Tu pedido</span><h2>Revisa antes de enviar</h2></div><button onClick={() => setCartOpen(false)}><X /></button></header><div className="public-cart-lines">{cart.map((item, index) => <div key={item.id}><div><strong>{item.name}</strong><span><Money value={item.price * item.quantity - promotionCalculation.lineDiscounts[index]} /></span>{promotionCalculation.lineDiscounts[index] > 0 && <small>Promoción: -<Money value={promotionCalculation.lineDiscounts[index]} /></small>}</div><div className="public-qty"><button onClick={() => change(item, -1)}><Minus /></button><span>{item.quantity}</span><button onClick={() => change(item, 1)}><Plus /></button></div></div>)}</div><div className="public-total public-total-breakdown"><span>Subtotal</span><strong><Money value={subtotal} /></strong>{promotionCalculation.discount > 0 && <><span>Promociones</span><strong className="public-discount">-<Money value={promotionCalculation.discount} /></strong></>}<span>Total de productos</span><strong><Money value={productTotal} /></strong></div><button className="public-primary" onClick={() => setCheckout(true)}>Continuar <Plus /></button></aside></div>}
    {checkout && <Modal title="Datos del pedido" onClose={() => setCheckout(false)}><div className="form-stack public-checkout"><div className="channel-switch"><button className={fulfillment === "takeaway" ? "active" : ""} onClick={() => setFulfillment("takeaway")}><ShoppingBag /> Recoger</button>{menu.branch.delivery_enabled && <button className={fulfillment === "delivery" ? "active" : ""} onClick={() => setFulfillment("delivery")}><Truck /> Delivery</button>}</div><label>Nombre<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>WhatsApp<input value={phone} onChange={(event) => setPhone(event.target.value)} /></label>{fulfillment === "delivery" && <label>Dirección y referencia<textarea value={address} onChange={(event) => setAddress(event.target.value)} /></label>}<label>Indicaciones<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ej. sin cubiertos" /></label>{promotionCalculation.discount > 0 && <div className="public-promotion-summary"><span>Ahorras con promociones</span><strong><Money value={promotionCalculation.discount} /></strong></div>}<div className="public-total"><span>Total estimado</span><strong><Money value={total} /></strong></div><button className="public-primary" onClick={() => void placeOrder()} disabled={submitting}>{submitting ? "Enviando..." : "Enviar pedido al restaurante"}</button><small>El restaurante confirmará disponibilidad y pago antes de preparar.</small></div></Modal>}
    {confirmation && <Modal title="Pedido recibido" onClose={() => setConfirmation(null)}><div className="public-confirmation"><CheckCircle2 /><h2>¡Gracias, {name}!</h2><p>Tu pedido quedó pendiente de confirmación.</p><strong><Money value={confirmation.total} /></strong><button className="public-primary" onClick={() => setConfirmation(null)}>Volver a la carta</button></div></Modal>}
  </main>;
}
